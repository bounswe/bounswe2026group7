#!/usr/bin/env python3
"""Retry failed mentor seeds — verifies email from mailhog with larger page."""
...
def get_verification_token(email):
    # Fetch up to 500 messages to handle large inboxes
    for _ in range(8):
...
for i, m in enumerate(FAILED):
    email = m["email"]
    print(f"[{i+1}/{len(FAILED)}] {m['firstName']} {m['lastName']}", end=" ... ", flush=True)
...
print(f"\nDone: {ok}/{len(FAILED)} retried successfully.")
